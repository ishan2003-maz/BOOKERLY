"use client";
import styles from "../../../../styles/admin/services/page.module.css"
...
export default function Services(){
  ...
  useEffect(() => {
    ...
    const loadBookings = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(buildApiUrl("/api/admin/service-bookings"));
        const data = await response.json();

        if (!response.ok) {
          throw new Error(data.message || "Unable to load service bookings.");
        }
    ...
    <div className={styles.container}>
      <aside className={styles.sidebar}>
        ...
        <ul className={styles.navList}>
          ...
          <a href="/components/admin/services"><li className={styles.active}><FaList /> Service Bookings</li></a>
      ...
      <main className={styles.mainContent}>
        <header className={styles.header}>
          ...
          <input
            type="text"
            placeholder="Search by service, user, or phone"
        ...
        <table className={styles.table}>
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan="6">Loading service bookings...</td>
              </tr>
            ) : filteredBookings.length === 0 ? (
              <tr>
                <td colSpan="6">No service bookings yet.</td>
              </tr>
            ) : (
              filteredBookings.map((booking) => (
                <tr key={booking.id}>
                  ...
                  <td>{booking.selectedDay} at {booking.selectedTime}</td>
                  <td>
                    {booking.createdAt
